...
import techplex.core.recipes.AlloyFurnaceRecipes;

public class TileEntityAlloyFurnace extends TileEntityInventory implements ISidedInventory, IUpdatePlayerListBox {
	public int burnTime;
	...
	public void update() {
		...
		if (burning != this.burnTime > 0) {
			this.markDirty();
			this.validate();
			AlloyFurnace.setState(this.isBurning(), this.worldObj, this.pos);
		}
	}
	...
	private boolean canSmelt()
	{
		...
		if (input != null) {
			ItemStack output = AlloyFurnaceRecipes.getInstance().getAlloyResult(input, input2);
			if (output == null)
				return false;
	...
	public void smeltItem() {
		if (this.canSmelt()) {
			...
			input.stackSize -= AlloyFurnaceRecipes.getInstance().getInputSize(input);
			if (input.stackSize <= 0)
				this.itemStacks[0] = null;

			input2.stackSize -= AlloyFurnaceRecipes.getInstance().getInput2Size(input2);
